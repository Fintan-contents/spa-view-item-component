...
import { useTodoGetView } from "./MxQueryButton.view";

/**
 * Material UIのButtonをラップしたコンポーネントであり、API呼び出し(useQuery)、ラベル表示、バリデーション表示、イベントハンドラなどの主要な機能が内部に実装されています。
 */
const meta: Meta<typeof MxQueryButton> = {
  title: "Material UI/MxQueryButton",
  component: MxQueryButton,
  tags: ["autodocs"],
  argTypes: {
    event: {
      control: false,
...
const defaultArgs: Partial<MxQueryButtonProps> = {
  event: undefined,
  type: "contained",
  children: "ボタン",
...
/**
 * MxQueryButtonコンポーネントのサンプル。
 */
export const MxQueryButtonStory: Story = {
  args: {
    ...defaultArgs,
  },

  render: function Render(args: MxQueryButtonProps) {
