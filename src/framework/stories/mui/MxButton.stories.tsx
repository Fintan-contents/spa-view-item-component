...
/**
 * Material UIのButtonをラップしたコンポーネントであり、ラベル表示、バリデーション表示、イベントハンドラなどの主要な機能が内部に実装されています。
 */
const meta: Meta<typeof MxButton> = {
  title: "Material UI/MxButton",
  component: MxButton,
  tags: ["autodocs"],
  argTypes: {
    type: {
...
const defaultArgs: Partial<MxButtonProps> = {
  onClick: () => {},
  type: "contained",
  children: "ボタン",
...
/**
 * MxButtonコンポーネントのサンプル。
 */
export const MxButtonStory: Story = {
  args: {
    ...defaultArgs,
  },
  render: function Render(args: MxButtonProps) {
    return (
      <>
        <div style={{ marginBottom: "30px" }}></div>
        <MxButton {...args}>{args.children}</MxButton>
        <div style={{ marginBottom: "30px" }}></div>
      </>
    );
  },
...
const CustomizeItemParameters = (args: MxButtonProps) => {
