...
import { useTodoPostView } from "./MxMutateButton.view";

/**
 * Material UIのButtonをラップしたコンポーネントであり、API呼び出し(useMutate)、ラベル表示、バリデーション表示、イベントハンドラなどの主要な機能が内部に実装されています。
 */
const meta: Meta<typeof MxMutateButton> = {
  title: "Material UI/MxMutateButton",
  component: MxMutateButton,
  tags: ["autodocs"],
  argTypes: {
    event: {
      control: false,
    ...
    type: {
      ...
      description:
        "ボタンの表示方法を指定します。指定しない場合にはMaterial UIのdefaultが指定されます。",
    },
    validationViews: {
      control: false,
    ...
    muiProps: {
      control: false,
      description: `Material UIのButtonコンポーネントに渡すプロパティを指定します。
                <a href="https://mui.com/components/buttons/" target="_blank">詳細</a><br><br>定義例：\`muiProps={{ styles: { color: "white" } }}\``,
      table: {
...
const defaultArgs: Partial<MxMutateButtonProps> = {
  event: undefined,
  type: "contained",
  children: "ボタン",
...
/**
 * MxMutateButtonコンポーネントのサンプル。
 */
export const MxMutateButtonStory: Story = {
  args: {
    ...defaultArgs,
    children: "ボタン",
  },

  render: function Render(args: MxMutateButtonProps) {
    ...
    return (
      <>
        ...
        <MxMutateButton {...args} event={todoPostView.createButton}>
          {args.children}
        </MxMutateButton>
        <div style={{ marginBottom: "30px" }}></div>
      </>
...
const CustomizeItemParameters = (args: MxMutateButtonProps) => {
  ...
  return (
    <>
      ...
      <MxMutateButton
        {...args}
        muiProps={{ disabled: disable.value }}
        event={todoPostView.createButton}
        validationViews={[todoPostView]}
        confirmOption={
          confirm.value
      ...
      <MxRadioBox item={messageOption} />
      <MxCheckBox item={disable} />
      <MxCheckBox item={confirm} />
    </>
  );
};
