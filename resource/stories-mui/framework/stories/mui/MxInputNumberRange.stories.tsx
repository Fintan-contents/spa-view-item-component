// MxInputNumberRange の Story ファイル
import {
  MxButton,
  MxCheckBox,
  MxInputNumberRange,
  MxInputNumberRangeProps,
  MxInputText,
} from "../../components/mui";
import {
  numberRule,
 ...
 */
const meta: Meta<typeof MxInputNumberRange> = {
  title: "Material UI/MxInputNumberRange",
  component: MxInputNumberRange,
  tags: ["autodocs"],
  argTypes: {
    item: {
      control: false,
    ...
    muiPropsLower: {
      control: false,
      description: `数値範囲の下限の最小値・最大値の指定ができます。<br><br>定義例：\`muiPropsLower={{inputProps: {min: 0, max: 100}}}\``,
    },
    muiPropsUpper: {
      control: false,
...
const defaultArgs: Partial<MxInputNumberRangeProps> = {
  item: undefined,
  hideLabel: false,
  labelPlacement: "top",
  labelWidth: 30,
...
/**
 * MxInputNumberRangeコンポーネントのサンプル。
 */
export const InputNumberRange: Story = {
  ...
  render: function Render(args) {
    ...
    return (
      <>
        <MxInputNumberRange {...args} item={item} />
      </>
    );
  },
  tags: ["!autodocs", "!dev"],
...
const CustomizeItemParameters = () => {
  ...
  return (
    <>
      <MxInputNumberRange item={targetItem} />
      <hr />
      <MxInputText item={label} />
      <MxInputNumberRange item={range} />
      <MxInputNumberRange item={init} />
